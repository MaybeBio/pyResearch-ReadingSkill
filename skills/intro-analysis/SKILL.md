...
1. 来源文献（完整引用格式）
2. 原文内容（保留原文英文）
3. 文献唯一标识（DOI或PMID，仅需ID）

### 3. 每主题深度展开 (Per-Theme Deep Dive) ⭐

**每个识别的主题必须获得独立的纵向深入分析**，不能浅尝辄止：
...
**纵向深入要求**：`[模型归纳]` 分析必须层层递进，不能停留在"这表明X方法有效"的表面总结。每个主题的分析需要：
- **纵向挖掘**：从序列→结构→功能→应用的因果链深入推演
- **跨文献关联**：不同文献的观点互相印证或矛盾，形成叙事线索
- **根源分析**：不止描述"有什么缺口"，要分析"为什么有缺口"、"缺口的根因是什么"
- **趋势推演**：基于时间线推演该主题的未来走向
...
**引用标识规则**：所有文献引用必须使用DOI或PMID作为唯一标识，禁止使用paper_01、paper_X等自编临时编号。理由：临时编号在不同分析中不一致，容易造成引用混乱和幻觉。
...
1.5. **DOI/PMID标识**：所有文献引用必须使用DOI或PMID，禁止paper_01等临时编号，避免幻觉
1.6. **主题纵向深入**：`[模型归纳]` 必须层层递进，不止于表面总结，要纵向挖掘因果链和根源分析
