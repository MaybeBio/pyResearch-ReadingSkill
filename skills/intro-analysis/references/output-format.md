...
- 观点来源标注：`(paper_id, Introduction第X段)`
...
```mermaid
graph TD
    A["「观点A」<br/>(paper_1, Intro Para X)"]:::base
    B["「观点B」<br/>(paper_2, Intro Para Y)"]:::derived
    A --> B
...
[重复每个主题——每个主题都必须包含上述全部8个部分...]

## 4. 观点关系网络 (Viewpoint Relationship Network)

### 4.1 观点依赖总览
[Mermaid graph - 展示所有观点的依赖关系，节点包含原文引用]

### 4.2 核心论证流程
[Mermaid flowchart - 展示论证逻辑链，节点包含原文引用]

### 4.3 观点演进序列
[Mermaid sequence diagram - 展示时间序列演进，消息包含原文引用]

### 4.4 观点对比表
[复杂表格 - 包含观点表述原文、标签、支持/反对、依赖关系、证据]

## 5. 研究缺口分析 (Research Gap Analysis)

### 5.1 缺口依赖图
[Mermaid graph - 展示缺口之间的依赖关系，节点包含原文引用和标签]

### 5.2 缺口对比表
[表格 - 缺口ID、描述原文、标签、提出文献、填补状态、依赖关系、争议性]

### 5.3 已被填补的缺口 (Filled Gaps)
### 5.4 当前主要缺口 (Current Major Gaps)
### 5.5 争议性缺口 (Controversial Gaps)

## 6. 文献逐篇分析 (Paper-by-Paper Analysis)

### 6.1 [完整引用格式1] - 完整分析（有Introduction）
- 研究定位 `[标签]` + 证据
- 历史脉络（每条都有原文引用）
- 研究现状（每条都有原文引用）
- 研究缺口（每条都有原文引用和标签）
...
## 7. 参考文献索引 (Reference Index)

### 8.1 文献元信息表
| ID | 标题 | 作者 | 年份 | 期刊 | DOI | PMID | 完整引用 |
|----|------|------|------|------|-----|------|---------|
| paper_1 | ... | ... | ... | ... | ... | ... | ... |
...
## 8. 可视化图例 (Visualization Legend)
解释所有Mermaid图例和表格结构

## 9. 反质量检查 (Anti-Quality Check)
...
- **引用格式**：完整引用，可追溯到原始文献
- **原文标注**：所有观点都有 `(id: paper_id, Introduction第X段)` 格式的标注
